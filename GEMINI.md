...
- **User-Centric:** The build process and project management should be as simple as possible for the end-user. We will use Meson for this purpose. When uncertain about a feature or implementation detail, ask for user input.

## 2. Environment and Tooling

### Build System
- This project uses **Meson**. All build configurations should be managed through `meson.build` files. The build directory (`build/`) is ignored by Git.
...
- **Commits:** Commit changes **frequently**. Each commit should represent a small, logical unit of work.
- **Pushing:** Push commits to the remote repository **often**. This ensures there is always a backup and allows for easy rollbacks by you or the user. A good cadence is after implementing any distinct feature or fix.

### Dependencies
- **Qt:** The project will use the Qt framework.
- **GitHub Source:** Dependencies may be fetched directly from GitHub repositories.

## 3. Available Resources

- You have access to all `gemini-cli` capabilities.
- You can utilize any packages already installed on the user's system.
